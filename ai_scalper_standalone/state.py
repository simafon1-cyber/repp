...
class SymbolState:
    ...
    # Почему НЕ сработал календарь новостей — отдельно от общей причины
    # отказа. Владелец: «ни разу новостная не работала». Без этого поля
    # отличить «новостей сейчас нет» от «источник календаря не отвечает»
    # было невозможно, и обе беды выглядели одинаково — молчанием.
    last_news_error: str = ""
    last_trade_result: str = "—"

    # Предупреждение о риске, которое человек ОБЯЗАН увидеть, а не найти в
    # журнале: минимальный лот брокера рискует больше, чем разрешает профиль.
    # На маленьком депозите это главная причина «очень много убытка»: сколько
    # ни настраивай процент риска, ниже минимального лота опуститься нельзя.
    last_risk_warning: str = ""
    last_buy_score: float = 0.0
    last_sell_score: float = 0.0

    # Сработало ли зеркало на последнем входе: сигнал был в одну сторону,
    # а заявка ушла в другую. Нужно, чтобы это было ВИДНО, а не только
    # лежало в журнале: человек, увидевший продажу при сигнале на
    # покупку, должен понимать, что это не сбой, а включённое им
    # «Зеркало». В хедж-режиме остаётся False — переворачивать там нечего.
    last_mirror_flip: bool = False
    ...
    # Те же последние сделки, но В ДЕНЬГАХ, а не «плюс/минус». Нужны, чтобы
    # отключить инструмент, который стабильно тянет счёт вниз: винрейт
    # 50% ничего не говорит, если пять плюсов по доллару перекрываются
    # одним минусом на тридцать. По реальному отчёту владельца одно золото
    # дало 85% всех потерь при винрейте 35% — см. symbol_loss_share().
    recent_profits: list = field(default_factory=list)
