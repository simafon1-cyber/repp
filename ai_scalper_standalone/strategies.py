...
STRATEGIES: list[Strategy] = [
    ...
    Strategy(
        key="peak_lock",
        title="Фиксация вверху",
        idea="стоп идёт за пиком прибыли и закрывает сделку при откате от него",
        when=(
            "когда важнее не отдать уже заработанное, чем заработать "
            "больше. Владелец: «вывесела сделка плюс двадцать долларов, и "
            "она просто откатилась до минус пять»"),
        caution=(
            "ПРОВЕРЕНО: В СРЕДНЕМ ЭТО СТОИТ ДОРОЖЕ, ЧЕМ ДАЁТ. Прогон по "
            "паспорту В-001 на 136 тысячах сделок (train и validation): "
            "выход по откату от пика даёт −0,065…−0,081 R на сделку "
            "против −0,059…−0,066 R у простого стопа с целью. То есть "
            "чуть ХУЖЕ, а не лучше, и хуже на всех проверенных долях "
            "отката. Причина известна: каждое вмешательство в открытую "
            "сделку срезает правый хвост — редкие большие плюсы, которыми "
            "и живёт стратегия. ЗАТО этот профиль защищает ровно от того "
            "случая, который не нравится владельцу: плюс двадцать, а "
            "закрылось в минус. Это выбор между «реже обидно» и «в "
            "среднем чуть лучше», а не улучшение"),
        params={
            # Простой выход выключается: пока он включён, программа не
            # прикасается к открытой сделке ВООБЩЕ, и никакая фиксация не
            # работает. Именно это и объясняет случай владельца, если у
            # него была выбрана С-001.
            "USE_SIMPLE_EXIT": False,
            # Цели нет — иначе сделка закроется по ней раньше, чем стоп
            # успеет пойти за пиком, и фиксация вверху не проявится.
            "USE_MAX_PROFIT_RIDE": True,
            # Сам механизм: стоп гонится за ПИКОВОЙ прибылью.
            "USE_PROFIT_LOCK_TRAILING": True,
            "USE_TIERED_PROFIT_LOCK": True,
            # Запирать начинаем с половины риска сделки, а не с целого:
            # владельцу важно не отдать уже набранное. Ниже половины
            # ставить нельзя — стоп упрётся в минимальную дистанцию
            # брокера и в спред.
            "PROFIT_LOCK_START_R_FRACTION": 0.5,
            # Чем выше был пик, тем больше от него запирается.
...
ЧЕРНОВИКИ = frozenset({"c001_simple", "mirror", "peak_lock"})
...
ПАСПОРТА = {"c001_simple": "strategy_c001.json",
            ...
            "peak_lock": "strategy_peak_exit.json"}
