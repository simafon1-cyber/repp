...
# Длина бара в секундах. Нужна, чтобы понять «бар сменился?» СЧЁТОМ, не
# спрашивая терминал.
TIMEFRAME_SECONDS = {
...
def timeframe_seconds(name: str) -> int:
    """Сколько секунд длится один бар. 0 — таймфрейм незнакомый."""
    return int(TIMEFRAME_SECONDS.get(str(name or "").upper().strip(), 0))


def bar_start(tick_time, seconds: int):
    """Во сколько начался бар, идущий ПРЯМО СЕЙЧАС. None — посчитать нельзя.

    ЗАЧЕМ ЭТО НУЖНО. Вход в сделку возможен только на новом баре. Чтобы
    узнать, сменился ли бар, программа раньше выкачивала у терминала 300
    баров и считала по ним все индикаторы — 8.4 мс на пару только на счёт,
    плюс сам запрос. И почти всегда результат выбрасывался: на M5 бар
    меняется раз в 300 секунд, а проход идёт каждые 5.

    Здесь то же самое получается делением. Отметка времени тика уже
    запрошена (она нужна для определения замершего рынка), а начало бара —
    это отметка, округлённая вниз до длины бара. Ни одного лишнего обращения
    к терминалу.

    Часовой пояс брокера роли не играет: отметка сравнивается сама с собой,
    как и в market_hours.note_quote."""
    try:
        moment = int(tick_time)
        step = int(seconds)
    except (TypeError, ValueError):
        return None
    if moment <= 0 or step <= 0:
        return None
