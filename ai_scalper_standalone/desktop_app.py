...
#
# Здесь подставляется честная заглушка: денежные вызовы кричат, вызовы
# подключения отвечают «не удалось», справочные отдают пустоту. Подробности
# и обоснование деления — в platform_support.py.
...
class App:
    ...
    def _предупредить_о_платформе(self):
        """Один раз сказать вслух, что торговля на этой машине невозможна.

        Показывается ПОСЛЕ того, как окно уже нарисовано: до этого момента
        сообщение легло бы поверх пустого экрана и выглядело бы как отказ
        запускаться, хотя всё остальное работает."""
        if ТОРГОВЛЯ_ВОЗМОЖНА:
            return
        try:
            messagebox.showwarning(
                "Торговля на этом компьютере недоступна",
                СОСТОЯНИЕ_ПЛАТФОРМЫ,
                parent=self.root,
            )
        except Exception:
            # Не смогли показать окно — уже записано в журнал при запуске.
            pass

    def run(self):
        # ПОСЛЕ отрисовки окна, ДО главного цикла ожидания.
        self.root.after(400, self._предупредить_о_платформе)
        self.root.mainloop()
