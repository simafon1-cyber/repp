...
ADVANCED_PARAMS = [
    ...
    # ПРЕДТОРГОВЫЙ БАРЬЕР. Раньше эти четыре настройки не показывались в
    # окне вовсе, и задать их можно было только правкой config.py в
    # блокноте — то есть ровно тем, чего владелец делать не должен. А без
    # них барьер закрыт (fail-closed): галочка «Реальная торговля» стоит,
    # а заявки всё равно не уходят, и причина ниоткуда не видна.
    #
    # Менять их УДАЛЁННО по-прежнему нельзя: они в списке FORBIDDEN в
    # remote_settings.py. Здесь окно на своём компьютере, а не дашборд.
    ("DEMO_ACCEPTANCE_MODE", "bool", "Демо-приёмка",
     "Предторговый барьер: пускать заявки только на заранее названный счёт", None),
    ("DEMO_ACCEPTANCE_LOGIN", "int", "Демо-приёмка",
     "Номер счёта, на который РАЗРЕШЕНО торговать (0 = не задан, торговля запрещена)", None),
    ("DEMO_ACCEPTANCE_SERVER", "str", "Демо-приёмка",
    ...
    ("USE_SIMPLE_EXIT", "bool", "Ведение сделки",
     "Простой выход: после входа НЕ трогать сделку (только SL/TP у брокера)", None),
    ("CLOSE_BOT_POSITIONS_ONLY", "bool", "Ведение сделки",
     "Кнопки «закрыть все» трогают только сделки бота, не ваши ручные", None),
...
CONFIG_SECTIONS = [
    ...
    ("Риск", ["Стопы / TP", "Ведение сделки", "Break Even", "Трейлинг-стоп", "Profit Lock",
                      "Просадка / серии убытков", "Издержки", "Частичное закрытие",
                      "Фиксация прибыли"]),
    ("Защита", ["Демо-приёмка", "Защитные проверки", "Новости (пороги)"]),
    ("AI-сигнал", ["AI-сигнал"]),
    ("Обучение", ["Автообучение", "Автообновление"]),
    ("Вид", ["Оформление"]),
]

# РАЗДЕЛЫ «НАСТРОЙКИ», КОТОРЫХ В ПРОСТОМ РЕЖИМЕ НЕТ.
#
# Владелец: «вкладки риски защита тоже можно убрать сигналы, обучение
# убрать… Делаем максимально минимализм в управлении программы».
#
# ПОЧЕМУ СПРЯТАНЫ, А НЕ УДАЛЕНЫ. Удалить страницу — значит навсегда
# лишить возможности вернуть значение обратно. Правило проекта прямое:
# «все правки торговой логики должны быть обратимыми». В разделе «Риск»
# лежат стопы и тейк-профиты, в «Защите» — демо-приёмка. Стереть к ним
# дорогу — это правка торговой логики, которую нельзя откатить.
#
# Поэтому сделано так же, как уже сделано для целых вкладок (см.
# ADVANCED_TAB_NAMES): в простом режиме их не видно совсем, а
# переключатель «Продвинутый» на «Обзоре» возвращает их на место.
# Значения при этом не меняются ни на одну цифру.
ADVANCED_CONFIG_SECTIONS = ("Риск", "Защита", "Обучение", "Профили")
...
class App:
    ...
    def _apply_ui_mode(self, initial: bool = False):
        ...
        for name in ADVANCED_TAB_NAMES:
            ...
            if is_advanced and not shown:
                ...
                book.hide(frame)
        # Разделы вкладки «Настройка» — тот же приём, что и для вкладок.
        книга = getattr(self, "config_sub", None)
        for имя in ADVANCED_CONFIG_SECTIONS:
            рамка = getattr(self, "config_section_frames", {}).get(имя)
            if рамка is None or книга is None:
                continue
            видна = str(рамка) in книга.tabs()
            if is_advanced and not видна:
                книга.add(рамка, text=имя)
            elif not is_advanced and видна:
                книга.hide(рамка)
    ...
    def _build_tab_overview(self, parent):
        ...
        buttons = [
            ...
            ("Разобрать происшествие", self.разобрать_происшествие),
            ("Разрешить этот счёт", self.разрешить_текущий_счёт),
        ]
    ...
    def _build_tab_config(self, parent):
        ...
        # Раньше эта страница называлась «Быстрый старт» и начиналась с
        # выбора риска. Выбора риска на ней больше нет (владелец: «убрать
        # выбор совсем»), осталась одна вещь — какой стратегией торговать.
        # Название должно говорить, что на странице лежит.
        self.config_sub = sub
        quick = ttk.Frame(sub)
        sub.add(quick, text="Стратегия")
        self._build_quick_setup(quick)

        # Точная настройка: каждый раздел — своя подвкладка.
        # Рамки запоминаются, чтобы простой режим мог их убрать и вернуть.
        self.param_vars = {}
        self.config_section_frames = {}
        for title, groups in CONFIG_SECTIONS:
            frame = ttk.Frame(sub)
            sub.add(frame, text=title)
            self.config_section_frames[title] = frame
            self._build_tab_params(frame, only_groups=groups)

        extra = ttk.Frame(sub)
        sub.add(extra, text="Профили")
        self.config_section_frames["Профили"] = extra
        self._build_tab_profiles_and_context(extra)

    # СТРАНИЦА «СТРАТЕГИЯ». Раньше называлась «Быстрый старт» и начиналась
    # с трёх карточек-режимов, задававших профиль риска.
    #
    # Владелец: «убрать выбор совсем», «оставить только выбор стратегии».
    # Карточки и список QUICK_PRESETS удалены целиком — вместе с кодом,
    # который «применял» уже стоявший профиль, то есть не менял ничего, но
    # мог упасть с ошибкой. Риск как был задан настройками, так и остался.
    def _build_quick_setup(self, parent):
        """Одна страница с одним выбором: какой стратегией торговать."""
        ttk.Label(parent, text="Чем торгуем",
                  font=("Segoe UI", 13, "bold")).pack(anchor="w", padx=12, pady=(14, 2))
        ttk.Label(parent, foreground=self.colors["muted"], wraplength=820,
                  justify="left",
                  text="Выберите стратегию и нажмите «Применить и запустить "
                       "бота». Больше на этой странице настраивать нечего.\n\n"
                       "Стратегия решает, ЧТО считать сигналом. Защиты счёта "
                       "она не отключает: обязательный стоп-лосс, риск на "
                       "сделку, потолок совокупного риска и снижение объёма "
                       "по серии убытков работают при любой стратегии."
    ...
    def _apply_strategy(self):
        ...
        # ЧЕРНОВИК КНОПКОЙ НЕ ПРИМЕНЯЕТСЯ.
        #
        # В списке выбора его и нет (см. strategies.titles), но запрет
        # стоит и здесь: ключ активной стратегии можно вписать в
        # config.py руками, а список — не единственный путь сюда.
        # Проверка, которую можно обойти, добравшись до кода ниже, —
        # не проверка.
        if strategies_mod.черновик(strategy):
            можно, почему = strategies_mod.паспорт_заверен(strategy)
            if not можно:
                messagebox.showwarning(
                    APP_TITLE,
                    f"«{strategy.title}» — ЧЕРНОВИК, а не готовая стратегия.\n\n"
                    f"Не пускает вот что: {почему}.\n\n"
                    "Её прибыльность НЕ проверена и НЕ обещана. Разбор истории "
                    "показал только, что простой выход теряет меньше сложного — "
                    "это не то же самое, что зарабатывает.\n\n"
                    "Чтобы её включить, по регламенту нужен заверённый паспорт "
                    "версии, а печать в нём должна сходиться с содержимым.\n\n"
                    "Кнопкой это не обходится. Ничего не изменено.")
                return
            # Паспорт на месте и печать сходится. Дальше — не запрет, а
            # честное предупреждение: регламент пройден, но обещания
            # прибыли как не было, так и нет.
            if not messagebox.askyesno(
                    APP_TITLE,
                    f"«{strategy.title}» — версия на ДЕМО-ПРИЁМКЕ.\n\n"
                    "Паспорт заверён, печать сходится — включать разрешено.\n\n"
                    "Что она делает: после входа программа НЕ ТРОГАЕТ сделку. "
                    "Работают только стоп и цель у брокера. Безубыток, "
                    "трейлинг, частичное закрытие — всё выключается.\n\n"
                    "Что она НЕ обещает: прибыль. Проверяется исправность "
                    "исполнения, а не заработок.\n\n"
                    "Включить?"):
                return

        params = strategies_mod.safe_params(strategy)

        # ФИКСАЦИЯ ПРИБЫЛИ ВЫКЛЮЧАЕТСЯ МОЛЧА — вот этого быть не должно.
        #
        # Владелец: «Фиксация прибыли должна быть на постоянной основе».
        # Но стратегия может её снять: простой выход (USE_SIMPLE_EXIT)
        # заставляет программу вообще не прикасаться к открытой сделке,
        # и тогда стоп за пиком не работает. Именно так и вышло однажды:
        # «плюс двадцать долларов, и она просто откатилась до минус пять».
        #
        # Запрещать выбор нельзя — это решение владельца. А вот молчать
        # о последствии нельзя тем более.
        снимает = []
        if params.get("USE_SIMPLE_EXIT") is True:
            снимает.append("простой выход: программа не будет прикасаться "
                           "к открытой сделке вообще")
        if params.get("USE_PROFIT_LOCK_TRAILING") is False:
            снимает.append("стоп перестанет идти за пиком прибыли")
        if снимает:
            if not messagebox.askyesno(
                APP_TITLE,
                f"«{strategy.title}» ВЫКЛЮЧИТ фиксацию прибыли.\n\n"
                + "\n".join(f"• {с}" for с in снимает)
                + "\n\nЭто значит: сделка сможет уйти в плюс и вернуться "
                  "в минус — стоп за ней не пойдёт.\n\n"
                  "Всё равно применить?"):
                return
    ...
    def _apply_quick_and_start(self):
        """Запустить бота. Ничего больше эта кнопка не делает.

        Раньше она сначала «применяла режим»: ставила профиль риска в то
        самое значение, которое уже стояло. Пользы ноль, а упасть с
        ошибкой это могло — и тогда бот не запускался вовсе."""
        try:
            self.start_bot()
            self.quick_status.configure(text="Бот запущен.",
                                        foreground=self.colors["profit"])
        except Exception as e:  # noqa: BLE001
            log.exception("Не удалось запустить бота со страницы «Стратегия»")
            messagebox.showerror(APP_TITLE, f"Бот не запустился: {e}")
    ...
    def _build_tab_settings(self, parent):
        ...
        ttk.Label(parent, foreground=self.colors["muted"], wraplength=800,
                  justify="left",
                  text="Здесь только показано. Чтобы менять, включите "
                       "«Обзор» → «Вид» → «Продвинутый»: тогда в "
                       "«Настройке» появится раздел «Риск»."
    ...
    def _build_tab_system(self, parent):
        ...
        # ---------- 0. Диспетчер демо: чей сейчас ход ----------
        # ЗАЧЕМ ЭТА РАМКА. Очередь версий задана заранее и лежит в
        # репозитории, но человек должен видеть, чей сейчас ход и сколько
        # осталось до конца окна. Иначе «диспетчер» — это файл, о котором
        # знает только исполнитель, а владелец опять смотрит на пустой
        # экран и не понимает, чего ждать.
        #
        # ПОЧЕМУ НЕ НА «ОБЗОРЕ». Там пять рамок и предел на число: они
        # про то, что происходит с торговлей ПРЯМО СЕЙЧАС. Очередь
        # версий — административное, её место рядом с выгрузкой и
        # обновлением. Проверка раскладки поймала это сразу.
        #
        # Кнопки здесь НЕ переключают версию по своему усмотрению:
        # завершение шага — именное действие человека, и диспетчер его
        # записывает, а не решает сам.
        disp = ttk.LabelFrame(parent, text=" Диспетчер демо — очередь версий ")
        disp.pack(fill="x", padx=12, pady=6)
        self.dispatcher_var = tk.StringVar(value="—")
        ttk.Label(disp, textvariable=self.dispatcher_var,
                  wraplength=820, justify="left").pack(
                      anchor="w", padx=10, pady=(6, 2))
        disp_row = ttk.Frame(disp)
        disp_row.pack(fill="x", padx=8, pady=6)
        for column, (текст, команда) in enumerate((
                ("Обновить", self._обновить_диспетчер),
                ("Записать отчёт шага", self._отчёт_шага_диспетчера),
                ("Завершить шаг вручную", self._завершить_шаг_вручную))):
            ttk.Button(disp_row, text=текст, command=команда).grid(
                row=0, column=column, padx=(0, 6))
        self._обновить_диспетчер()
    ...
    def _состояние_диспетчера(self):
        """(каталог, состояние, решение). Ничего не меняет."""
        каталог = dispatcher.прочитать_каталог()
        состояние = dispatcher.прочитать_состояние()
        return каталог, состояние, dispatcher.решение(каталог, состояние)

    def _обновить_диспетчер(self):
        """Что показать человеку про очередь версий."""
        try:
            каталог, состояние, реш = self._состояние_диспетчера()
        except Exception as e:  # noqa: BLE001
            log.exception("Диспетчер не опрошен: %s", e)
            self.dispatcher_var.set(f"Диспетчер не отвечает: "
                                    f"{type(e).__name__}: {e}")
            return
        шаг = dispatcher.шаг_по_id(каталог, реш.get("шаг", ""))
        строки = [f"Сейчас: {реш.get('шаг') or '—'} — {реш.get('что')}",
                  реш.get("почему", "")]
        if шаг:
            окно = шаг.get("окно") or {}
            минут = int(каталог.get("минут_для_полного_дня")
                        or dispatcher.МИНУТ_ДЛЯ_ПОЛНОГО_ДНЯ)
            строки.append(
                f"Окно: дней {dispatcher.полных_дней(состояние, реш['шаг'], минут)}"
                f" из {окно.get('минимум_полных_торговых_дней', '?')}, "
                f"сделок {dispatcher.закрытых_сделок(состояние, реш['шаг'])}"
                f" из {окно.get('минимум_закрытых_сделок', '?')}"
                f" (полный день — от {минут} минут работы без помех)")
        строки.append("Версию по убытку диспетчер не меняет: прибыль в его "
                      "решение не входит вовсе.")
        self.dispatcher_var.set("\n".join(с for с in строки if с))

    def _отчёт_шага_диспетчера(self):
        try:
            каталог, состояние, реш = self._состояние_диспетчера()
            путь = dispatcher.отчёт_шага(
                каталог, состояние, реш.get("шаг", ""),
                замороженные_пары=sorted(self.symbols_tree.get_children())
                if getattr(self, "symbols_tree", None) else ())
        except Exception as e:  # noqa: BLE001
            log.exception("Отчёт шага не записан: %s", e)
            messagebox.showerror(APP_TITLE, f"Отчёт не записан: {e}")
            return
        if путь:
            messagebox.showinfo(APP_TITLE, f"Отчёт шага записан:\n{путь}")
        else:
            messagebox.showwarning(APP_TITLE, "Отчёт записать не вышло — "
                                              "смотрите лог.")
        self.отметить_нажатие("Записать отчёт шага")

    def _завершить_шаг_вручную(self):
        """Завершение шага — ИМЕННОЕ действие человека, не решение программы."""
        try:
            каталог, состояние, реш = self._состояние_диспетчера()
        except Exception as e:  # noqa: BLE001
            messagebox.showerror(APP_TITLE, f"Диспетчер не отвечает: {e}")
            return
        id_шага = реш.get("шаг", "")
        if not id_шага:
            messagebox.showinfo(APP_TITLE, "Сейчас нет шага, который можно "
                                           "завершить.")
            return
        if not messagebox.askyesno(
                APP_TITLE,
                f"Завершить шаг «{id_шага}» вручную?\n\n"
                f"{реш.get('почему', '')}\n\n"
                "Это записывается как ваше решение, с вашим именем и "
                "временем. Диспетчер сам версию не меняет и по убытку её "
                "менять не станет.\n\n"
                "Завершить?"):
            return
        получилось, пояснение = dispatcher.завершить_шаг(
            состояние, id_шага, dispatcher.ВРУЧНУЮ, кто="владелец",
            комментарий="завершено кнопкой в окне программы",
            подтверждено_владельцем=True)
        messagebox.showinfo(APP_TITLE, пояснение) if получилось else \
            messagebox.showwarning(APP_TITLE, пояснение)
        self._обновить_диспетчер()
        self.отметить_нажатие("Завершить шаг вручную")

    def разрешить_текущий_счёт(self):
        """Вписать в барьер тот счёт, к которому подключён терминал сейчас.

        ЗАЧЕМ ЭТА КНОПКА. Барьер требует, чтобы номер счёта и сервер были
        записаны заранее. У владельца счета меняются, и каждый новый
        закрывал барьер наглухо: «ожидаемый счёт не задан — торговать
        нельзя». Он видел это трижды и трижды терял на этом день.

        Вписать номер руками можно было и раньше — но для этого надо
        знать, где он лежит, и не ошибиться в имени сервера. Одна кнопка
        берёт и то и другое у терминала.

        ЧТО ЭТА КНОПКА НЕ ДЕЛАЕТ. Она не снимает требование «счёт
        демонстрационный»: на реальном счёте она откажет и скажет почему.
        И она не включает торговлю — только называет барьеру счёт."""
        snap = ds.get_snapshot()
        acc = (snap or {}).get("account") or {}
        номер = acc.get("login")
        сервер = str(acc.get("server") or "")
        if not номер or not сервер:
            messagebox.showinfo(
                APP_TITLE,
                "Пока не видно, к какому счёту подключён терминал.\n\n"
                "Нажмите «Старт» и подождите несколько секунд: номер счёта "
                "программа узнаёт на первом круге работы.")
            return

        режим = acc.get("trade_mode")
        только_демо = bool(getattr(cfg, "DEMO_ACCEPTANCE_REQUIRE_DEMO", True))
        if только_демо and режим is not None and int(режим) != pretrade_gate.СЧЁТ_ДЕМО:
            имя = pretrade_gate.ИМЯ_РЕЖИМА.get(int(режим), f"неизвестный ({режим})")
            messagebox.showwarning(
                APP_TITLE,
                f"Счёт {номер} — {имя}, а не демонстрационный.\n\n"
                "Разрешены только демонстрационные счета. Эта кнопка на "
                "реальном счёте ничего не меняет — и не должна.")
            return

        if not messagebox.askyesno(
                APP_TITLE,
                f"Разрешить барьеру работать на счёте {номер} "
                f"({сервер})?\n\n"
                "Номер и сервер будут записаны в настройки. Заявки станут "
                "уходить только на этот счёт — подключитесь к другому, и "
                "барьер снова закроется.\n\n"
                "Требование «счёт должен быть демонстрационным» остаётся.\n\n"
                "Записать?"):
            return
        try:
            _write_config_value("DEMO_ACCEPTANCE_LOGIN", repr(int(номер)))
            _write_config_value("DEMO_ACCEPTANCE_SERVER", repr(сервер))
            _reload_cfg()
        except Exception as e:  # noqa: BLE001
            log.exception("Счёт в барьер не записан: %s", e)
            messagebox.showerror(APP_TITLE, f"Не записалось: {e}")
            return
        messagebox.showinfo(
            APP_TITLE,
            f"Готово: барьер настроен на счёт {номер} ({сервер}).\n\n"
            "Перезапустите программу — барьер проверяется один раз при "
            "подключении к терминалу.")
        self.отметить_нажатие("Разрешить этот счёт")

    def разобрать_происшествие(self):
        """Выяснить, чем кончились неподтверждённые заявки, и снять запрет.

        ЗАЧЕМ ЭТА КНОПКА. Механизм разбора был написан целиком: программа
        умеет сама сходить на счёт и посмотреть позиции, активные заявки,
        историю заявок и историю сделок, а снять запрет — только если исход
        ДОКАЗАН. Но вызвать его было нечем: ни кнопки в окне, ни на
        дашборде. У владельца торговля встала на «в журнале осталось 4
        отправленных заявок, которым НЕ нашлось позиции на счету», и снять
        это он не мог никак.

        Сама программа запрет не снимает нигде и никогда — ни при старте,
        ни по времени. Только человек и только именно."""
        if not control.инцидент():
            messagebox.showinfo(
                APP_TITLE,
                "Незакрытых происшествий нет — разбирать нечего.\n\n"
                "Если сделок всё равно нет, посмотрите верхние строки на "
                "«Обзоре»: там написано, чего не хватает.")
            return

        try:
            вердикт = control.сверить_инцидент()
        except Exception as e:  # noqa: BLE001
            log.exception("Разбор происшествия не удался: %s", e)
            messagebox.showerror(
                APP_TITLE,
                f"Разобрать не вышло: {type(e).__name__}: {e}\n\n"
                "Запрет остаётся. Это правильно: пока не выяснено, "
                "торговать нельзя.")
            return

        факты = "\n".join(f"  • {ф}" for ф in вердикт.факты) or "  • фактов нет"
        шапка = f"Что выяснено: {вердикт.состояние}\n\nФакты со счёта:\n{факты}"
        if вердикт.совет:
            шапка += f"\n\n{вердикт.совет}"

        if not вердикт.доказан:
            # ИСХОД НЕ ДОКАЗАН — но выход должен быть.
            #
            # Владелец: «мне не нужно, чтобы бот постоянно стоял в
            # запрете». И он прав: запрет, который нельзя снять, — это не
            # предохранитель, а поломка. Особенно когда номера заявки в
            # отметке нет и доказать связь НЕЧЕМ в принципе.
            #
            # Поэтому здесь не тупик, а развилка: программа честно
            # говорит, что доказательства у неё нет, и спрашивает
            # человека, проверил ли он счёт сам. Снятие записывается как
            # ПРИНУДИТЕЛЬНОЕ — чтобы через месяц было видно, на чём оно
            # держалось.
            if not messagebox.askyesno(
                    APP_TITLE,
                    шапка + "\n\n"
                    "ИСХОД НЕ ДОКАЗАН — сама программа запрет не снимет.\n\n"
                    "«Позиции сейчас не видно» доказательством не является: "
                    "заявка может быть ещё активной или исполниться позже. "
                    "Открыть новую сделку поверх неучтённой хуже, чем "
                    "простоять.\n\n"
                    "ЕСЛИ ВЫ УЖЕ ПРОВЕРИЛИ СЧЁТ В ТЕРМИНАЛЕ и видите, что "
                    "лишних позиций и активных заявок нет, — запрет можно "
                    "снять под вашу ответственность. В журнале это "
                    "запишется как СНЯТ ПРИНУДИТЕЛЬНО, вместе со списком "
                    "заявок, которые вы списали.\n\n"
                    "Снять запрет под вашу ответственность?"):
                messagebox.showinfo(
                    APP_TITLE,
                    "Запрет оставлен. Новые сделки не открываются.\n\n"
                    "Ничего не изменено.")
                return

            кто = "владелец (снял принудительно, проверив счёт в терминале)"
            снято_заявок = 0
            try:
                # ЗАПИСИ УБИРАЮТСЯ ВМЕСТЕ С ЗАПРЕТОМ. Иначе при следующем
                # запуске сверка снова их не найдёт, снова откроет
                # инцидент и снова остановит торговлю — то есть снятие
                # ничего не даст.
                import pending_orders
                снято_заявок = pending_orders.снять_человеком(
                    кто, "принудительное снятие из окна программы")
            except Exception as e:  # noqa: BLE001
                log.exception("Не удалось снять ожидаемые заявки: %s", e)

            снят = control.снять_инцидент(
                кто=кто,
                комментарий=f"исход не доказан ({вердикт.состояние}); "
                            f"списано заявок: {снято_заявок}")
            if снят:
                messagebox.showinfo(
                    APP_TITLE,
                    f"Запрет снят принудительно. Списано неподтверждённых "
                    f"заявок: {снято_заявок}.\n\n"
                    "Записано в журнал происшествий и в лог — с перечнем "
                    "заявок.\n\n"
                    "Если какая-то из них всё же исполнится позже, позиция "
                    "появится на счету, а программа о ней знать не будет. "
                    "Присматривайте за терминалом ближайшее время.")
            else:
                messagebox.showwarning(
                    APP_TITLE, "Снять не вышло. Запрет остаётся.")
            self.отметить_нажатие("Разобрать происшествие")
            return

        if not messagebox.askyesno(
                APP_TITLE,
                шапка + "\n\nИсход ДОКАЗАН. Снять запрет и разрешить "
                        "новые сделки?"):
            return

        снят, вердикт2 = control.разрешить_инцидент(
            кто="владелец (кнопка «Разобрать происшествие»)",
            комментарий="разбор из окна программы")
        if снят:
            messagebox.showinfo(
                APP_TITLE,
                "Запрет снят. Снятие записано в журнал происшествий — "
                "видно, когда и на основании чего.")
        else:
            messagebox.showwarning(
                APP_TITLE,
                f"Снять не вышло: {вердикт2.состояние}. Запрет остаётся.")
        self.отметить_нажатие("Разобрать происшествие")

    def _почему_нет_сделок(self, snap: dict) -> list:
        """Причины, по которым сделок не будет ВООБЩЕ — до разбора по парам.

        Жалоба владельца, дословно: «все сделал как надо выше, сделки не
        вижу». Программа в этот момент знала ответ, но держала его в трёх
        разных местах: состояние цикла — на «Обзоре», барьер — строкой в
        логе при запуске, список пар — на «Символах», а выключенная
        реальная торговля не показывалась нигде. Человеку приходилось
        обходить вкладки и догадываться.

        _silence_reasons отвечает на вопрос «почему не входим ПО ПАРАМ» и
        работает только когда пары уже есть и цикл уже сделал круг. Здесь —
        причины, которые делают сделки невозможными до всякого разбора пар,
        и каждая написана вместе с тем, что нажать."""
        строки = []

        if not (self.bot_thread and self.bot_thread.is_alive()):
            строки.append(
                "СДЕЛОК НЕ БУДЕТ: торговый цикл не запущен. "
                "Нажмите «Старт» здесь, на «Обзоре».")

        if not bool(getattr(cfg, "LIVE_TRADING", False)):
            строки.append(
                "СДЕЛОК НЕ БУДЕТ: «Реальная торговля» выключена — программа "
                "только считает и пишет в журнал, заявки брокеру не уходят. "
                "Включается: «Настройка» → «Торговля» → «Общее» → "
                "«Реальная торговля».")

        # Барьер спрашивают у самого модуля, а не у снимка: снимок делает
        # торговый цикл, а барьер закрывается ДО первого круга — тогда
        # снимка ещё нет, и причина не показалась бы вовсе.
        if pretrade_gate.включён(cfg) and not pretrade_gate.открыт():
            номер, сервер, _ = pretrade_gate.ожидаемый_счёт(cfg)
            чего_нет = []
            if not номер:
                чего_нет.append("номер счёта (DEMO_ACCEPTANCE_LOGIN)")
            if not сервер:
                чего_нет.append("имя сервера (DEMO_ACCEPTANCE_SERVER)")
            точная = pretrade_gate.причина_отказа()
            if чего_нет:
                хвост = " Не задано: " + ", ".join(чего_нет) + "."
            elif точная:
                # Причина словами, а не отсылка к файлу журнала: искать её
                # в логе владельцу пришлось ровно один раз, и этого хватило.
                хвост = " " + точная
            else:
                хвост = (" Счёт, сервер или режим не совпали с заданными — "
                         "точная причина в логе при запуске.")
            строки.append(
                "СДЕЛОК НЕ БУДЕТ: предторговый барьер закрыт, ни одна заявка "
                "уйти не может." + хвост +
                " Задаётся так: «Обзор» → «Вид» → «Продвинутый», затем "
                "«Настройка» → «Защита» → «Демо-приёмка».")

        if snap and not (snap.get("symbols") or []):
            строки.append(
                "СДЕЛОК НЕ БУДЕТ: не отобрано ни одной пары. "
                "Посмотрите вкладку «Символы» — список пуст.")
    ...
    def _состояние_торговли(self, snap, помехи=None):
    ...
    def _refresh_loop(self):
        try:
            snap = ds.get_snapshot()
            # ПОМЕХИ СЧИТАЮТСЯ ПЕРВЫМИ, а состояние — уже зная о них.
            # Иначе выходит враньё: сверху «Торговля разрешена, помех нет»,
            # а строкой ниже «СДЕЛОК НЕ БУДЕТ: барьер закрыт». Владелец
            # прислал снимок экрана ровно с этим.
            помехи = self._почему_нет_сделок(snap if isinstance(snap, dict) else {})
            состояние = self._состояние_торговли(snap, помехи)
            ...
            # Причины «сделок не будет вообще» идут ПЕРВЫМИ и не зависят от
            # снимка: без него они как раз и нужнее всего.
            problems = list(помехи) + list(problems)
    ...
    def _upload_journal_if_due(self):
        ...
        # СОСТОЯНИЕ ПРОГРАММЫ СОБИРАЕТСЯ ЗДЕСЬ, В ПОТОКЕ ОКНА.
        #
        # Владелец: «все ошибки, которые появляются, пускай он записывает
        # где-то в логе на гитхабе, чтобы ты его сам непосредственно мог
        # прочитать этот файл». Присылать снимки экрана он больше не
        # должен.
        #
        # Почему сбор именно тут, а не в рабочем потоке: барьер и
        # происшествие живут в модулях окна, и трогать их из чужого потока
        # — лишний повод для гонки. Сбор дешёвый, отправка — нет, поэтому
        # уезжает в поток ниже.
        состояние_текст = ""
        try:
            барьер_сейчас = {
                "открыт": pretrade_gate.открыт(),
                "причина_отказа": pretrade_gate.причина_отказа(),
                "разрешение": pretrade_gate.разрешение(),
            } if pretrade_gate.включён(cfg) else {
                "открыт": True,
                "причина_отказа": "",
                "разрешение": "барьер выключен",
            }
            состояние_текст = cloud_diag.собрать(
                снимок=snapshot,
                барьер=барьер_сейчас,
                инцидент=control.инцидент(),
                ожидающих=len(pending_orders.открытые() or []),
                путь_журнала=LOG_FILE,
                версия=app_version.short())
        except Exception:  # noqa: BLE001
            # Диагностика не имеет права мешать ни торговле, ни журналу.
            log.debug("Не удалось собрать состояние для выгрузки",
                      exc_info=True)
        ...
        def worker():
            ...
            if состояние_текст:
                try:
                    ок, почему = cloud_diag.выгрузить(состояние_текст, snapshot)
                    if not ок:
                        log.debug("Состояние не выложено: %s", почему)
                except Exception:  # noqa: BLE001
                    log.debug("Выгрузка состояния сорвалась", exc_info=True)
...
def selftest() -> int:
    ...
    ПОЧЕМУ ВСЁ НУЖНОЕ ЛЕЖИТ ВНУТРИ. Это последняя проверка перед тем, как
    сборка уедет к человеку. Зависеть от других функций модуля ей нельзя:
    сломается сосед — и проверка не отработает вовсе. Ровно поэтому
    безопасная печать ниже вложена сюда, а не вынесена наружу."""

    def напечатать(текст: str) -> None:
        """Напечатать так, чтобы это НИКОГДА не уронило самопроверку.

        ДЕФЕКТ, КОТОРЫЙ ЭТО ЧИНИТ, БЫЛ МОИМ. Сборка 85 упала вот так:

            UnicodeEncodeError: 'charmap' codec can't encode characters

        Самопроверка печатала русские слова и названия стратегий, а на
        сборочной машине Windows вывод перенаправляется в файл с
        кодировкой cp1252 — русских букв в ней нет вовсе. Печать бросала
        исключение, оконная программа показывала окно с ошибкой, нажать в
        нём «ОК» было некому, и сборка убивалась по трёхминутному сроку.
        Дважды.

        Обиднее всего, ЧТО именно упало: проверка, добавленная чтобы
        ловить ошибки ДО владельца, сама стала ошибкой, доехавшей до
        сборки."""
        try:
            print(текст)
            return
    ...
    try:
        ...
        напечатать(f"SELFTEST FAILED: {type(e).__name__}: {e}")
        return 1
    ...
    try:
        ...
        напечатать(f"SELFTEST FAILED: проверка копии: {type(e).__name__}: {e}")
        return 1

    # СПИСОК СТРАТЕГИЙ В СОБРАННОЙ ПРОГРАММЕ.
    #
    # Владелец: «у меня также самая не появилась стратегия зеркала».
    # Черновик показывается в списке только если рядом лежит его паспорт
    # и печать в нём сходится. В собранной программе паспорта попадают
    # внутрь через --add-data; забыли строку в сборке — и стратегия молча
    # исчезает у человека, а на машине разработчика всё на месте.
    #
    # Ловим это ЗДЕСЬ, на сборочном сервере: не нашлось паспорта — сборка
    # неудачная и до владельца не доезжает.
    try:
        import strategies as _st
        нет_паспорта = []
        for _ключ in _st.ПАСПОРТА:
            _можно, _почему = _st.паспорт_заверен(_ключ)
            if not _можно:
                нет_паспорта.append(f"{_ключ}: {_почему}")
        if нет_паспорта:
            напечатать("SELFTEST FAILED: паспорта не найдены или печать не "
                  "сходится: " + "; ".join(нет_паспорта))
            return 1
        _названия = _st.titles()
        _ждём = [_st.by_key(_к).title for _к in _st.ПАСПОРТА
                 if _st.by_key(_к) is not None]
        _нет = [_и for _и in _ждём if _и not in _названия]
        if _нет:
            напечатать(f"SELFTEST FAILED: стратегий нет в списке: {_нет}; "
                  f"есть только {_названия}")
            return 1
        напечатать(f"SELFTEST: стратегий в списке {len(_названия)}: {_названия}")
    except Exception as e:       # noqa: BLE001
        напечатать(f"SELFTEST FAILED: список стратегий: {type(e).__name__}: {e}")
        return 1

    напечатать("SELFTEST OK")
    return 0
...
def main():
    ...
    if "--selftest" in sys.argv:
        # НИ ОДНО ИСКЛЮЧЕНИЕ НЕ ИМЕЕТ ПРАВА ВЫЙТИ ОТСЮДА.
        #
        # Программа собрана оконной. Если она падает необработанным
        # исключением, Windows показывает окно с ошибкой — а нажать в нём
        # «ОК» на сборочной машине некому. Сборка 85 именно так и висела:
        # три минуты, потом ещё три, и только затем провал без внятной
        # причины.
        #
        # Провалившаяся проверка обязана вернуть КОД, а не окно.
        try:
            _код = selftest()
