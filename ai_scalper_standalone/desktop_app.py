...
def selftest() -> int:
    ...
    try:
        ...
        if нет_паспорта:
            print("SELFTEST FAILED: паспорта не найдены или печать не "
        ...
        if _нет:
            print(f"SELFTEST FAILED: стратегий нет в списке: {_нет}; "
                  f"есть только {_названия}")
            return 1
        print(f"SELFTEST: стратегий в списке {len(_названия)}: {_названия}")
    except Exception as e:       # noqa: BLE001
        print(f"SELFTEST FAILED: список стратегий: {type(e).__name__}: {e}")
        return 1
