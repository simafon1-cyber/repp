...
#
# Что взято от macOS, а что нет — честно. Взята ПАЛИТРА: тёплый светло-серый
# фон вместо голубоватого, белые карточки, мягкий синий системного акцента
# (#007aff), приглушённые серые подписи, зелёный и красный из системных
# цветов Apple. Не взято и взято быть не может: скруглённые углы, тени,
# полупрозрачность и шрифт SF Pro — всё это рисует сама операционная система,
# а окно здесь построено на обычных элементах Windows.
#
# То есть это «цвета в духе macOS», а не подделка macOS. Обещать второе было
# бы неправдой.
MACOS = {
    "name": "macos",
    "bg": "#ececec",          # тёплый серый рабочей области
    "card": "#ffffff",
    "fg": "#1d1d1f",          # почти чёрный, как системный текст Apple
    "muted": "#4b4b50",       # подписи
    "dim": "#6e6e73",         # второстепенное — системный серый
    "accent": "#0064d2",      # системный синий, притемнён ради контраста
    "accent_fg": "#ffffff",
    "profit": "#1d7a3a",      # системный зелёный, притемнён ради читаемости
    "loss": "#c9302c",        # системный красный, притемнён
    "warning": "#8a5a00",
    "tab_bg": "#e0e0e2",
    "tab_active": "#ffffff",
    "heading": "#e0e0e2",
...
THEMES = {"light": LIGHT, "dark": DARK, "macos": MACOS}

# Список для выбора в окне: имя в настройках и подпись человеку.
CHOICES = (
    ("light", "Светлая"),
    ("dark", "Тёмная"),
    ("macos", "В стиле macOS"),
...
def choices() -> tuple:
    """Что предлагать человеку. Имена берутся из THEMES, поэтому добавить
    тему и забыть показать её в окне невозможно."""
    return tuple((key, title) for key, title in CHOICES if key in THEMES)
DEFAULT = "light"
...
CONTRAST_RULES = [
    ...
    # ДОБАВЛЕНО ПОСЛЕ АУДИТА ИНТЕРФЕЙСА.
    #
    # Проверка была неполной. Цвета в трёх темах оказались в порядке —
    # но десять пар, которые человек видит каждый день, никем не
    # проверялись. То есть завтрашняя правка оттенка «чтобы красивее»
    # могла сделать нечитаемой таблицу позиций, и ни один тест бы не
    # возразил.
    #
    # Особенно это про card и row_alt: прибыль и убыток в таблице
    # позиций рисуются НЕ на фоне окна, а на фоне карточки и
    # чередующейся строки. Проверялись же они только на фоне окна.
    ("fg", "row_alt", 4.5),        # текст в чередующейся строке таблицы
    ("muted", "card", 4.5),        # подпись внутри карточки
    ("dim", "card", 3.0),          # второстепенное внутри карточки
    ("profit", "card", 3.0),       # прибыль в таблице позиций
    ("loss", "card", 3.0),         # убыток в таблице позиций
    ("profit", "row_alt", 3.0),    # прибыль в чередующейся строке
    ("loss", "row_alt", 3.0),      # убыток в чередующейся строке
    ("warning", "card", 3.0),      # предупреждение в карточке
    ("fg", "tab_bg", 4.5),         # надпись на НЕактивной вкладке
    ("muted", "heading", 4.5),     # подпись в шапке таблицы
]
