...
class Control:
    ...
    def инструменты_инцидента(self) -> set:
        """По каким инструментам инцидент запрещает вход.

        Пустое множество означает «неизвестно, по каким» — и тогда
        запрещено ВЕЗДЕ. Так и должно быть: повреждённая отметка или
        потерянная запись не говорят, где именно беда."""
        with self._lock:
            сведения = dict(self._инцидент_сведения or {})
        if сведения.get("состояние"):
            return set()          # отметка повреждена — не знаем ничего
        имена = set()
        один = str(сведения.get("символ") or "").strip()
        if один and один != "?":
            имена.add(один)
        for з in (сведения.get("неподтверждённые") or ()):
            if isinstance(з, dict):
                имя = str(з.get("символ") or "").strip()
                if имя and имя != "?":
                    имена.add(имя)
        return имена

    def вход_запрещён(self, symbol: str = "") -> tuple:
        """(запрещён_ли, почему) — ПО ЭТОМУ инструменту.

        ПОЧЕМУ НЕ ОДНА ОБЩАЯ ПАУЗА НА ВСЁ. Незакрытая заявка по CADMXN
        останавливала торговлю по ВСЕМ инструментам сразу. Опасность же
        ровно одна: открыть вторую позицию поверх возможной неучтённой —
        и она касается только того инструмента, по которому заявка
        зависла. Остановка всего счёта не защищала ни от чего сверх этого,
        зато обесценивала программу: владелец получил три дня без единой
        сделки.

        Обычная пауза с пульта по-прежнему останавливает всё: её ставит
        человек, и он имеет в виду именно всё.

        Если инцидент есть, а инструменты неизвестны — запрещено везде."""
        with self._lock:
            пауза = self._paused
            инцидент = self._инцидент
        if пауза:
            return True, "Пауза (управление с дашборда/телефона)"
        if not инцидент:
            return False, ""
        имена = self.инструменты_инцидента()
        if not имена:
            return True, "Незакрытое происшествие (инструмент не назван)"
        if str(symbol or "") in имена:
            return True, f"Незакрытое происшествие по {symbol}"
        return False, ""
