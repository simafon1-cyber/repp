...
def calc_lot(symbol: str, sl_dist: float, equity: float, sym_state: SymbolState) -> float:
    ...
    # ПОТОЛОК ПЛЕЧА — второй, независимый ограничитель объёма.
    #
    # Потолок выше считает риск ПО СТОПУ: лот × расстояние до стопа. Он молча
    # предполагает, что стоп исполнится ровно там, где стоит. У узкого стопа
    # это предположение ломается: расширился спред, проскочила цена — и
    # потеряно кратно больше расчётного, потому что лот-то огромный.
    #
    # А лот именно огромный: объём считается как риск/расстояние-до-стопа, и
    # чем УЖЕ стоп, тем БОЛЬШЕ лот. Ночью, когда волатильность падает, стоп
    # сжимается — и объём разносит.
    #
    # Так и вышло в отчёте за 06-07.08.2026 (счёт $65). USDCAD днём: средний
    # лот 0.029, средний убыток 0.43. Ночью: средний лот 0.147 (в 5 раз
    # больше), средний убыток 2.46 (в 5.7 раза больше). Лот 0.20 на счёте $65
    # — это позиция на $20 000, плечо 307:1. Все восемь сделок с лотом выше
    # 0.065 дали вместе -6.58 при итоге счёта -19.99.
    #
    # Здесь ограничивается не потеря, а РАЗМЕР позиции относительно счёта:
    # сколько бы ни обещал расчёт, объём больше equity × плечо не пройдёт. В
    # отличие от потолка риска сделка не отменяется, а УМЕНЬШАЕТСЯ до
    # допустимой — вход остаётся, исчезает только раздутость.
    max_leverage = float(getattr(cfg, "MAX_POSITION_LEVERAGE", 0) or 0)
    if max_leverage > 0 and equity > 0:
        contract = float(getattr(info, "trade_contract_size", 0) or 0)
        price = mt5c.get_price(symbol) or 0.0
        if contract > 0 and price > 0:
            notional_per_lot = contract * price
            allowed = equity * max_leverage / notional_per_lot
            allowed = math.floor(allowed / lot_step) * lot_step
            if allowed < final_lot:
                if allowed < min_lot:
                    # Даже минимальный лот брокера превышает потолок плеча.
                    # Отменять сделку здесь нельзя: за «слишком дорогой для
                    # депозита инструмент» уже отвечает потолок риска выше,
                    # и он считает по реальной цене стопа, а не по объёму.
                    # Иначе на маленьком счёте мы бы запретили вообще всё.
                    sym_state.last_risk_warning = (
                        f"Мин. лот {min_lot:.2f} по {symbol} — это плечо "
                        f"{min_lot * notional_per_lot / equity:.0f}:1 при потолке "
                        f"{max_leverage:.0f}:1. Уменьшить объём уже некуда.")
                    log.warning("%s: %s", symbol, sym_state.last_risk_warning)
                else:
                    log.info("%s: объём урезан с %.2f до %.2f — потолок плеча "
                             "%.0f:1 (позиция %.0f при счёте %.2f)",
                             symbol, final_lot, allowed, max_leverage,
                             final_lot * notional_per_lot, equity)
                    final_lot = allowed
...
def effective_target_money(profile: dict, equity: float) -> float:
    ...
    ЗАЧЕМ. В профилях цель задана абсолютным числом: у «Истерички» это 1.0,
    то есть один доллар. Число подбиралось под депозит $65, где оно означало
    полтора процента счёта. На депозите $1000 тот же один доллар — это одна
    десятая процента, и цель прибыли оказывается КРОШЕЧНОЙ: чем больше счёт,
    тем больше объём при том же проценте риска, а значит тем меньше движения
    цены нужно, чтобы «заработать доллар». Сделка закрывалась бы на паре
    пунктов, которые съедает спред.

    Абсолютные суммы в настройках вообще не переживают смену размера счёта —
    в отличие от процентов, которые масштабируются сами. Поэтому цель
    задаётся долей от эквити (TARGET_PROFIT_PERCENT_OF_EQUITY), а старое
    абсолютное число остаётся запасным вариантом для тех, кому нужна именно
    фиксированная сумма.

    ВАЖНО ПРО СЕГОДНЯШНИЙ ДЕНЬ: при USE_MAX_PROFIT_RIDE = True сделка
    открывается вообще без цели прибыли, и эта функция не вызывается. Она
    написана не потому, что сейчас что-то сломано, а потому что при смене
    депозита выключение «тянуть максимальную прибыль» дало бы тихую
    неприятность."""
    percent = float(getattr(cfg, "TARGET_PROFIT_PERCENT_OF_EQUITY", 0) or 0)
    fallback = float(profile.get("target_profit_money", 0) or 0)
    scaled = equity * percent / 100.0

    # Берём БОЛЬШЕЕ из двух. Одна строка закрывает сразу три случая, и
    # отдельных проверок на них не нужно (я их сначала написал, а потом убрал
    # как мёртвые):
    #   * настройка выключена (percent = 0) -> scaled = 0 -> вернётся прежнее
    #     абсолютное число, поведение ровно как раньше;
    #   * счёт неизвестен (equity = 0) -> то же самое;
    #   * счёт мал -> доля процента может оказаться меньше цены одного пункта,
    #     и цель потеряла бы смысл; абсолютное число служит полом.
    # Уменьшить цель по сравнению с прежним поведением эта функция не может
