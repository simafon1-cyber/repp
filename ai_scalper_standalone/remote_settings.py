...
FORBIDDEN = {
    ...
    # и поставлен. Задаётся только на своём компьютере: в окне программы
    # (вкладка «Настройка» → «Защита» → «Демо-приёмка») или в config.py.
    "DEMO_ACCEPTANCE_MODE", "DEMO_ACCEPTANCE_LOGIN",
    "DEMO_ACCEPTANCE_SERVER", "DEMO_ACCEPTANCE_REQUIRE_DEMO",
    # Зеркало переворачивает СТОРОНУ каждой заявки. Кто может включить
    # его с телефона — может развернуть всю торговлю в противоположную
    # сторону одним запросом, и человек за компьютером этого не увидит.
    # Задаётся только у себя: выбором стратегии «Зеркало» в окне
    # программы или строкой MIRROR_SIGNALS в config.py.
    "MIRROR_SIGNALS",
}
