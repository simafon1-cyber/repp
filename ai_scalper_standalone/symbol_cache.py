...
# Версия 2: в замер добавлен раздел брокера ("path"). Файлы версии 1 его не
# содержат, а без раздела нельзя отличить валютную пару от акции — и запрет
# на акции обходился через старый файл. Поэтому старый файл не читается, а
# перемеряется: лишние секунды при запуске против торговли не тем.
VERSION = 2
