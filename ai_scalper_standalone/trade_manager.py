...
def execute_market_order(symbol, direction, lot, sl_dist, tp_dist, score, point):
    ...
    for attempt in range(1, cfg.ORDER_RETRY_ATTEMPTS + 1):
        ...
        if not ожидание:
            # ЗАПИСЬ НЕ ЛЕГЛА НА ДИСК — ЗАЯВКУ НЕ ОТПРАВЛЯЕМ.
            #
            # Отправить сейчас значит создать позицию, о которой не
            # останется следа: программу могут закрыть между отправкой и
            # ответом, и следующий запуск не узнает, что мы вообще что-то
            # слали. Ровно тот случай, ради которого журнал и написан.
            #
            # Ложная пауза неприятна, но новой позиции она не создаёт.
            # Заявка без следа — создаёт.
            текст = (f"{symbol}: журнал ожидаемых заявок не записался на "
                     f"диск. Заявка НЕ отправлена — иначе позиция могла бы "
                     f"остаться без следа. Проверьте место на диске и права "
                     f"на папку программы.")
            log.error(текст)
...
def close_leg(symbol: str, ticket: int, direction: int = 0,
    ...
    for попытка in range(1, ПОПЫТОК_ЗАКРЫТИЯ + 1):
        ...
        if нога is None and (not ticket or ticket == TICKET_НЕИЗВЕСТЕН):
            # Номера нет — закрывать нечего, и подбирать «похожую» позицию
            # нельзя: под приметы подходит и чужая сделка.
            log.error("%s: номер ноги неизвестен — закрывать по приметам "
                      "отказываюсь", symbol)
            return ex.неизвестно(
                f"номер ноги неизвестен (искали {'BUY' if direction == 1 else 'SELL'} "
                f"объёмом {искомый_объём:.2f}) — закрывать похожую позицию "
                f"нельзя, это может оказаться чужая сделка", искомый_объём)
        if нога is None:
            if закрыто_всего > 0:
        ...
        if итог.статус == ex.ПОЛНОЕ:
            # ОТВЕТ «ЗАКРЫТО» — ЕЩЁ НЕ ЗАКРЫТО. Раньше здесь верили коду
            # DONE на слово и заканчивали. Но это тот же самый обман, что
            # и в И1-C, только в другую сторону: ответ брокера — не факт
            # состояния счёта. Проверяем, что позиции действительно не
            # стало.
            подтверждение = mt5c.positions_or_none(symbol=symbol,
                                                   magic=cfg.MAGIC_NUMBER)
            if подтверждение is None:
                return ex.неизвестно(
                    f"брокер ответил «закрыто» по ноге {нога.ticket}, но "
                    f"подтвердить это нечем: список позиций недоступен",
                    осталось)

            всё_ещё = next((p for p in подтверждение
                            if int(p.ticket) == int(нога.ticket)), None)
            if всё_ещё is None:
                note_manual_close(нога.ticket)
                закрыто_всего += осталось
                log.info("%s: нога %s закрыта и это подтверждено "
                         "(компенсация неполного хеджа)", symbol, нога.ticket)
                return ex.полное(нога.ticket, закрыто_всего)

            # Позиция на месте. Возможно, закрылась часть — тогда следующий
            # круг доберёт остаток; возможно, не закрылось ничего.
            ушло = осталось - float(всё_ещё.volume)
            if ушло > ex.ДОПУСК_ОБЪЁМА:
                закрыто_всего += ушло
            log.warning("%s: брокер ответил «закрыто» по ноге %s, но позиция "
                        "на месте (было %.2f, стало %.2f), попытка %d",
                        symbol, нога.ticket, осталось,
                        float(всё_ещё.volume), попытка)
            ticket = нога.ticket
            искомый_объём = 0.0
            continue
...
def _найти_ногу(позиции, ticket: int, direction: int, volume: float):
    """Нужная позиция среди наших — СТРОГО ПО НОМЕРУ.

    ЗДЕСЬ БЫЛ ЗАПАСНОЙ ПУТЬ ПО ПРИМЕТАМ: если номер неизвестен, бралась
    наша позиция нужного направления, подходящая по объёму, а из
    нескольких — самая свежая. Это тот же самый порок, что И1-D исправил
    в сверке входа, только последствия здесь хуже.

    Там угадывание приводило к неверному учёту. Тут — к ЗАКРЫТИЮ ЧУЖОЙ
    ПОЗИЦИИ: под приметы одинаково подходит сделка второго экземпляра
    программы и сделка, открытая человеком руками. Закрыть её означало бы
    распорядиться чужими деньгами по совпадению признаков.

    Поэтому без номера ответа нет. Вызывающий обязан это признать и
    остановиться, а не закрывать что-нибудь похожее.

    direction и volume остались в подписи: они больше ничего не выбирают,
    но по ним видно, что именно искали, — это попадает в сообщение
    человеку."""
    if not ticket or ticket == TICKET_НЕИЗВЕСТЕН:
        return None
    return next((p for p in позиции if int(p.ticket) == int(ticket)), None)
