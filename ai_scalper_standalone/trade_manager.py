...
def сверить_вход(symbol: str, direction: int, снимок, момент_сервера: float,
    ...
    # РАЗВИЛКА ПО ГЛАВНОМУ ПРИЗНАКУ: есть ли у нас номер заявки.
    #
    # Раньше здесь сначала искалась ЛЮБАЯ новая позиция нужного
    # направления, и только потом смотрели на номер. Из-за этого при
    # известном номере, но отсутствии позиции с таким номером, за нашу
    # принималась единственная чужая — и программа отвечала «открылось»,
    # снимала защиту и шла дальше, хотя настоящая заявка оставалась живой
    # и исполнялась позже. Две позиции, одна не учтена.
    #
    # Совпадение по инструменту, направлению и «она тут одна» — это
    # приметы, а не связь. Связь бывает только одна:
    #
    #     ЗАЯВКА  →  СДЕЛКА ВХОДА  →  НОМЕР ПОЗИЦИИ
    #
    # Если её не удаётся проследить, честный ответ — «неизвестно».
    if подсказка_тикет and подсказка_тикет > 0:
        return _разобрать_по_номеру(symbol, подсказка_тикет, заказано, позиции)

    return _разобрать_без_номера(symbol, direction, снимок, момент_сервера,
                                 заказано, позиции)


def _разобрать_по_номеру(symbol: str, ticket: int, заказано: float, позиции):
    """Чем кончилась ЗАЯВКА, номер которой известен.

    Порядок вопросов не случаен: сначала «была ли по ней сделка», потом
    «жива ли заявка», и только потом «чем она закончилась». Каждый
    следующий вопрос задаётся, лишь если предыдущий не дал ответа.

    НИ В ОДНОМ из этих вопросов не участвуют приметы позиции. Позиция
    принимается за нашу ТОЛЬКО по номеру, полученному из сделки по этой
    заявке."""
    # 1. СДЕЛКИ ПО ЭТОЙ ЗАЯВКЕ. Единственный путь от заявки к позиции.
    сделки = mt5c.deals_by_order(ticket)
    if сделки is None:
        return ex.неизвестно(
            f"ответ брокера неясен, историю по заявке {ticket} прочитать "
            f"не удалось", заказано)
    ...
    if входы:
        ...
        if len(номера) != 1:
            return ex.неизвестно(
                f"по заявке {ticket} сделки указывают на разные позиции "
                f"({sorted(номера) or 'номера нет'}) — автоматика такое не "
                f"разбирает", заказано)

        номер_позиции = номера.pop()
        объём = sum(_объём_сделки(d) for d in входы)

        позиция = next((p for p in позиции
                        if int(p.ticket) == номер_позиции), None)
        if позиция is None:
            return ex.неизвестно(
                f"заявка {ticket} исполнилась (объём {объём:.2f}) в позицию "
                f"{номер_позиции}, а среди открытых её нет — значит, её уже "
                f"закрыли. Нужен человек, автоматика такое не разбирает",
                заказано)

        статус = (ex.ПОЛНОЕ if объём >= заказано - ex.ДОПУСК_ОБЪЁМА
                  else ex.ЧАСТИЧНОЕ)
        log.warning("%s: ответ брокера был неясен — заявка %s прослежена до "
                    "позиции %s на %.2f из %.2f",
                    symbol, ticket, номер_позиции, объём, заказано)
        return ex.Итог(статус, номер_позиции, заказано, объём,
                       пояснение=f"заявка {ticket} прослежена до позиции "
                                 f"{номер_позиции} через сделку")

    # 2. ЖИВА ЛИ ЗАЯВКА ПРЯМО СЕЙЧАС. Тот случай, ради которого всё и
    #    переписано: заявка лежит на сервере, позиции ещё нет.
    активные = mt5c.order_by_ticket(ticket)
    if активные is None:
        return ex.неизвестно(
            f"ответ брокера неясен, список активных заявок получить не "
            f"удалось (заявка {ticket})", заказано)
    if активные:
        состояние = getattr(активные[0], "state", None)
        return ex.неизвестно(
            f"заявка {ticket} ещё жива на сервере "
            f"({mt5c.имя_состояния(состояние)}) — позиции по ней пока нет, "
            f"но отказа тоже не было. Никакая другая позиция за неё "
            f"засчитана быть не может", заказано)

    # 3. ЧЕМ ЗАЯВКА ЗАКОНЧИЛАСЬ. Единственное место, где отказ доказуем.
    завершённые = mt5c.history_order_by_ticket(ticket)
    if завершённые is None:
        return ex.неизвестно(
            f"ответ брокера неясен, историю заявок по номеру {ticket} "
            f"прочитать не удалось", заказано)
    if not завершённые:
        return ex.неизвестно(
            f"заявка {ticket} не найдена ни среди активных, ни среди "
            f"завершённых — чем она кончилась, неизвестно", заказано)

    состояние = getattr(завершённые[0], "state", None)
    if состояние in mt5c.СОСТОЯНИЯ_БЕЗ_ИСПОЛНЕНИЯ:
        log.info("%s: заявка %s закончилась ничем (%s) — сделки не было",
                 symbol, ticket, mt5c.имя_состояния(состояние))
        return ex.Итог(ex.ОТКАЗ, 0, заказано, 0.0,
                       пояснение=f"заявка {ticket} {mt5c.имя_состояния(состояние)}, "
                                 f"сделок по ней нет")

    if состояние in mt5c.СОСТОЯНИЯ_С_ИСПОЛНЕНИЕМ:
        return ex.неизвестно(
            f"заявка {ticket} помечена как {mt5c.имя_состояния(состояние)}, "
            f"но сделки входа по ней не видно", заказано)

    return ex.неизвестно(
        f"заявка {ticket} в состоянии «{mt5c.имя_состояния(состояние)}» — "
        f"окончательного ответа ещё нет", заказано)


def _разобрать_без_номера(symbol: str, direction: int, снимок,
                          момент_сервера: float, заказано: float, позиции):
    """Чем кончилась заявка, номер которой брокер не прислал.

    ЗДЕСЬ НИЧЕГО ДОКАЗАТЬ НЕЛЬЗЯ. Без номера у сервера нечего спросить:
    ни найти заявку среди активных, ни узнать её исход, ни проследить
    цепочку до позиции. Остаются приметы, а приметы — не доказательство.

    Поэтому итог всегда НЕИЗВЕСТНО, и появившаяся рядом позиция ничего не
    меняет: она похожа на нашу, но «похожа» и «наша» — разные слова. Разница
    лишь в том, что сказать человеку: чем точнее описано положение дел, тем
    быстрее он разберётся."""
    нужный_тип = _тип_позиции(direction)
    ...
    if новые:
        ...
        объёмы = ", ".join(f"{float(p.volume):.2f}" for p in новые)
        return ex.неизвестно(
            f"брокер не прислал номер заявки, а рядом появились новые "
            f"позиции ({номера}, объём {объёмы}). Похожи на нашу, но "
            f"доказать это нечем: без номера заявки связь не проследить. "
            f"Проверьте счёт в терминале", заказано)

    сделки = _история_после(symbol, момент_сервера, direction)
    if сделки is None:
        return ex.неизвестно(
            "ответ брокера неясен, номера заявки нет, историю сделок "
            "прочитать не удалось", заказано)
    ...
    активные = mt5c.orders_or_none(symbol=symbol, magic=cfg.MAGIC_NUMBER)
    if активные is None:
        подсказка = "список активных заявок получить не удалось"
    elif активные:
        номера = ", ".join(str(getattr(o, "ticket", "?")) for o in активные)
        подсказка = f"на сервере есть активные заявки: {номера}"
    else:
        подсказка = "активных заявок по инструменту не видно"

    return ex.неизвестно(
        f"брокер не прислал номер заявки, а ни позиции, ни сделки пока не "
        f"видно ({подсказка}). Отсутствие следов НЕ доказывает отказ: "
        f"заявка могла ещё не дойти до исполнения. Проверьте счёт в "
        f"терминале", заказано)
