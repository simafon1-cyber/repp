...
def сверить_вход(symbol: str, direction: int, снимок, момент_сервера: float,
    ...
    # Новых позиций нет. Это НАБЛЮДЕНИЕ, а не доказательство отказа.
    #
    # ГЛАВНОЕ ПРАВИЛО ЭТОЙ ЧАСТИ (И1-C). Между «отправлено» и «исполнено»
    # заявка какое-то время просто ЛЕЖИТ на сервере. В этот момент позиции
    # ещё нет и сделки в истории ещё нет — а отказа не было: заявка жива и
    # вот-вот сработает. Раньше код на этом месте отвечал «отказ», снимал
    # защиту и шёл открывать следующую сделку. Через секунду первая
    # исполнялась, и риск оказывался двойным.
    #
    # Особенно очевидно это для ответа PLACED (10008): он так и переводится
    # — «заявка размещена». Не «отклонена».
    #
    # Поэтому дальше мы ищем ДОКАЗАТЕЛЬСТВО, а не отсутствие следов.
    # Отказ разрешён ровно в одном случае: заявка найдена в истории и её
    # состояние — отменена, отклонена или истекла, и сделок по ней нет.
    if подсказка_тикет and подсказка_тикет > 0:
        return _разобрать_по_номеру(symbol, подсказка_тикет, заказано)

    return _разобрать_без_номера(symbol, direction, снимок,
                                 момент_сервера, заказано)


def _разобрать_по_номеру(symbol: str, ticket: int, заказано: float):
    """Чем кончилась ЗАЯВКА, когда её номер известен.

    Порядок вопросов не случаен: сначала «была ли сделка», потом «жива ли
    заявка», и только потом «чем она закончилась». Каждый следующий вопрос
    задаётся, лишь если предыдущий не дал ответа."""
    # 1. СДЕЛКИ ПО ЭТОЙ ЗАЯВКЕ. Если они есть — исполнение было, а позиции
    #    среди открытых нет: значит, её уже закрыли. Разбирать такое
    #    автоматикой нельзя.
    ...
    if входы:
        ...
        return ex.неизвестно(
            f"заявка {ticket} исполнилась (объём {объём:.2f}), а позиции "
            f"среди открытых нет — значит, её уже закрыли. Нужен человек, "
            f"автоматика такое не разбирает", заказано)

    # 2. ЖИВА ЛИ ЗАЯВКА ПРЯМО СЕЙЧАС. Это и есть тот случай, ради которого
    #    всё переписано: заявка лежит на сервере, позиции ещё нет.
    ...
    if активные:
        ...
        return ex.неизвестно(
            ...
            f"({mt5c.имя_состояния(состояние)}) — позиции пока нет, но "
            f"отказа тоже не было. Ждать её исполнения вслепую нельзя",
            заказано)
    ...
    if not завершённые:
        # Заявки нет ни среди активных, ни среди завершённых. Такого быть
        # не должно; скорее всего история просто не успела обновиться.
        return ex.неизвестно(
    ...
    if состояние in mt5c.СОСТОЯНИЯ_С_ИСПОЛНЕНИЕМ:
        # Заявка исполнена, но ни позиции, ни сделок мы не нашли. Одно из
        # двух врёт или запаздывает — сами разобраться не можем.
        return ex.неизвестно(
            f"заявка {ticket} помечена как {mt5c.имя_состояния(состояние)}, "
            f"но ни позиции, ни сделок по ней не видно", заказано)
...
def _разобрать_без_номера(symbol: str, direction: int, снимок,
                          момент_сервера: float, заказано: float):
    """Чем кончилась заявка, когда её номер брокер не прислал.

    ЗДЕСЬ ОТКАЗ НЕДОКАЗУЕМ В ПРИНЦИПЕ. Без номера нечего спросить у
    сервера: нельзя ни найти заявку среди активных, ни посмотреть, чем она
    закончилась. Остаётся смотреть по приметам, а приметы доказательством
    не являются — их отсутствие тем более.

    Поэтому итог всегда НЕИЗВЕСТНО. Разница лишь в том, что сказать
    человеку: чем точнее описано положение дел, тем быстрее он разберётся."""
    ...
    # Ни позиции, ни сделки. Смотрим активные заявки по инструменту: если
    # там что-то есть, человеку сразу видно, за чем следить.
