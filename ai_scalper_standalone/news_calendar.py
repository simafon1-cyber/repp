...
def explain_news_entry(symbol: str, window_minutes: int = 0) -> str:
    ...
    только объясняет."""
    # Режимов торговли больше нет: новостной вход работает всегда, а когда
    # свежей новости нет — обычный отбор сигнала. Выключить его отдельным
    # переключателем нельзя, потому что такого переключателя не осталось.
    if not getattr(cfg, "USE_NEWS_FILTER", True):
