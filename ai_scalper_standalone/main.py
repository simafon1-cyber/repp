...
def process_symbol(symbol: str, sym_state: SymbolState, acc_state: AccountState, equity: float,
    ...
    # ================================================================
    # ЗЕРКАЛО: ВХОД В ПРОТИВОПОЛОЖНУЮ СТОРОНУ
    # ================================================================
    # Владелец, дословно: «может просто попробовать сделать все в
    # зеркале, то есть сигнал приходит на покупку пусть ставит на
    # продажу». Отдельная стратегия «Зеркало», по умолчанию ВЫКЛЮЧЕНА.
    #
    # ПОЧЕМУ ПЕРЕВОРОТ ИМЕННО ЗДЕСЬ И ТОЛЬКО ЗДЕСЬ. Это единственное
    # место, где готово окончательное направление и где ещё ничего не
    # отправлено брокеру. Переворачивать раньше — значит менять сам
    # сигнал и его оценку; переворачивать позже — значит иметь в
    # программе два разных мнения о том, куда мы идём, и однажды
    # поставить стоп не с той стороны.
    #
    # ЧТО ЗЕРКАЛО НЕ МЕНЯЕТ: ни один фильтр, ни один порог, ни стоп, ни
    # цель, ни размер лота. Стоп и цель считаются ПОСЛЕ этой строки и
    # уже от перевёрнутого направления — то есть встают правильно.
    #
    # ЧЕСТНАЯ ОГОВОРКА ПРО ХЕДЖ: в хедж-режиме открываются обе стороны
    # сразу, [1, -1]. Зеркало превращает его в [-1, 1] — тот же набор.
    # Это не ошибка: у симметричного входа зеркала не существует.
    if bool(getattr(cfg, "MIRROR_SIGNALS", False)):
        было = list(directions_to_open)
        directions_to_open = [-d for d in directions_to_open]
        if set(было) != set(directions_to_open):
            log.info("ЗЕРКАЛО %s: сигнал %s -> открываем %s",
                     symbol,
                     "BUY" if было[0] == 1 else "SELL",
                     "SELL" if было[0] == 1 else "BUY")
            sym_state.last_mirror_flip = True
        else:
            # Хедж: набор не изменился. Молчать об этом нельзя — иначе
            # человек будет думать, что зеркало работает, а оно не имеет
            # на что подействовать.
            log.info("ЗЕРКАЛО %s: хедж открывает обе стороны, "
                     "переворачивать нечего", symbol)
            sym_state.last_mirror_flip = False
