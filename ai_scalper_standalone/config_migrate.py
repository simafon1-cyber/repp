...
ONE_TIME = [
    (
        ...
        "фиксация прибыли включена НАСОВСЕМ. Решение владельца 03.09.2026, "
        "дословно: «Фиксация прибыли должна быть на постоянной основе». "
        "Стоп теперь идёт за ПИКОМ прибыли и начинает запирать с половины "
        "риска сделки, а не с целого. Заодно снят простой выход "
        "(USE_SIMPLE_EXIT): пока он включён, программа не прикасается к "
        "открытой сделке ВООБЩЕ, и никакая фиксация не работает — именно "
        "это и объясняет случай «плюс двадцать, закрылось в минус пять». "
        "ЧЕСТНО: замер по паспорту В-001 показал, что в среднем такая "
        "защита стоит дороже, чем даёт (−0,065…−0,081 R против "
        "−0,059…−0,066 R). Она защищает от обидного случая, а не "
        "улучшает средний результат. Правка одноразовая: выключите "
        "обратно — так и останется",
    ),
    (
