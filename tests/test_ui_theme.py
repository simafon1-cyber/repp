...
def test_черновик_стратегии_не_попадает_в_список_выбора():
    ...
    import os
    import shutil
    черновые = [s for s in strategies.STRATEGIES if strategies.черновик(s)]
    check(черновые, "Черновики в наборе вообще есть",
          str([s.title for s in черновые]))
    # Убираем паспорта по-настоящему: черновик без заверенного паспорта
    # предлагать нельзя. С паспортом — можно, это и проверяет отдельный
    # набор test_c001_passport.
    убранные = []
    for s_ in черновые:
        путь = strategies.путь_паспорта(s_)
        if путь and os.path.exists(путь):
            shutil.move(путь, путь + ".на_время_проверки")
            убранные.append(путь)
    try:
        видимые = strategies.titles()
        for s_ in черновые:
            check(s_.title not in видимые,
                  f"«{s_.title}» без заверенного паспорта не в списке выбора")
    finally:
        for путь in убранные:
            shutil.move(путь + ".на_время_проверки", путь)
    видимые = strategies.titles()
