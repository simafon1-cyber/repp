...
def test_кнопка_разрешить_счёт_не_трогает_реальный():
    """Кнопка вписывает в барьер текущий счёт. На РЕАЛЬНОМ она обязана
    отказать: иначе один щелчок открывал бы дорогу к настоящим деньгам."""
    print("\n[Кнопка «Разрешить этот счёт»]")
    import ast
    исходник = (APP / "desktop_app.py").read_text(encoding="utf-8")
    дерево = ast.parse(исходник)
    тело = None
    for узел in ast.walk(дерево):
        if isinstance(узел, ast.FunctionDef) and узел.name == "разрешить_текущий_счёт":
            тело = ast.unparse(узел)
    check(тело is not None, "Обработчик кнопки существует")
    if not тело:
        return
    check("СЧЁТ_ДЕМО" in тело,
          "Режим счёта сверяется с демонстрационным", тело[:80])
    check("DEMO_ACCEPTANCE_REQUIRE_DEMO" in тело,
          "И учитывается требование «только демо»")
    check("askyesno" in тело, "У человека спрашивают подтверждение")
    # Ни в коем случае не должна трогать сам запрет и торговлю.
    for запрет in ("DEMO_ACCEPTANCE_REQUIRE_DEMO\", False",
                   "LIVE_TRADING"):
        check(запрет not in тело,
              f"Кнопка не трогает {запрет.split(chr(92))[0]}", тело[:60])
