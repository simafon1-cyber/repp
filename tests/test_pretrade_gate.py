...
def test_программа_не_успокаивает_словом_демо():
    """Решение владельца 01.09.2026: «все счета считать за реал».

    Раньше программа говорила «счёт демонстрационный, всё в порядке».
    Успокаивать она больше не должна — ни на демо, ни тем более на реальном."""
    print("\n[Про счёт говорится честно]")
    cfg = настройки()
    g, _ = собрать(cfg, ПоддельныйТерминал(счёт=счёт()))

    текст = g.предупреждение_о_счёте(счёт(trade_mode=0))
    check("как с реальным" in текст,
          "На демо сказано, что ведём себя как с реальным", текст)
    check("всё в порядке" not in текст, "И никакого «всё в порядке»", текст)

    текст = g.предупреждение_о_счёте(счёт(login=777, trade_mode=2))
    check("НАСТОЯЩИЕ ДЕНЬГИ" in текст,
          "На реальном счёте сказано прямо про настоящие деньги", текст)
    check("777" in текст, "И назван номер счёта", текст)

    текст = g.предупреждение_о_счёте(счёт(trade_mode=None))
    check("РЕАЛЬНЫМ" in текст,
          "Режим неизвестен — считаем реальным, а не демо", текст)
