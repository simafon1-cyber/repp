...
def test_reservation_is_made_only_after_the_broker_confirms() -> None:
    ...
    Раньше это проверялось по тексту исходника — искалась строка
    «if ok:» между отправкой и бронью. Такая проверка ломается от
    переименования переменной и ничего не говорит о поведении. Теперь
    ордер подделывается, и считаются НАСТОЯЩИЕ брони."""
    print("\n[Бронь — только на подтверждённый ордер]")
    import trade_manager as tm
    настоящий = tm.execute_market_order
    профиль = rm.get_profile()
    был_порог = профиль["min_score_to_trade"]
    профиль["min_score_to_trade"] = 0.0      # иначе сделка не дойдёт до брони
    # Пауза — глобальная переменная в памяти, и её мог включить любой из
    # тестов выше. Снимаем явно, иначе проверка была бы зелёной по неверной
    # причине: брони нет не из-за отказа брокера, а из-за паузы.
    from control import control
    была_пауза = control.is_paused()
    control.set_paused(False)

    try:
        # 1. Брокер отказал — брони быть не должно.
        tm.execute_market_order = lambda *a, **k: ex.отказ("брокер отказал")
        сч = свежий_счёт()
        прогнать("EURUSD", сч, [])
        check(len(сч.reservations) == 0,
              "Отказ брокера брони не оставляет", str(len(сч.reservations)))

        # 2. Ответ неясен — брони тоже нет: сделки, возможно, не существует.
        tm.execute_market_order = lambda *a, **k: ex.неизвестно("ответа нет")
        сч = свежий_счёт()
        прогнать("EURUSD", сч, [])
        check(len(сч.reservations) == 0,
              "Неясный ответ брони не оставляет", str(len(сч.reservations)))
        check(control.is_paused(),
              "И невыясненный ответ ОСТАНАВЛИВАЕТ торговлю")
        # Пауза только что включилась по делу — снимаем, иначе следующая
        # проверка была бы зелёной по неверной причине.
        control.set_paused(False)

        # 3. КОНТРОЛЬ: подтверждённый ордер бронь ставит. Без этой проверки
        # первые две были бы зелёными и при полностью сломанной брони.
        tm.execute_market_order = lambda *a, **k: ex.полное(777, 0.01)
        сч = свежий_счёт()
        причина = прогнать("EURUSD", сч, [])
        check(len(сч.reservations) == 1,
              "А подтверждённый ордер бронируется",
              f"броней {len(сч.reservations)}, причина {причина!r}")

        # 4. Частичное исполнение — позиция ЕСТЬ, значит бронь нужна.
        tm.execute_market_order = lambda *a, **k: ex.Итог(
            ex.ЧАСТИЧНОЕ, 778, 0.02, 0.01)
        сч = свежий_счёт()
        прогнать("EURUSD", сч, [])
        check(len(сч.reservations) == 1,
              "Частично исполненная сделка тоже бронируется",
              str(len(сч.reservations)))
    finally:
        tm.execute_market_order = настоящий
        профиль["min_score_to_trade"] = был_порог
        control.set_paused(была_пауза)


def test_reservation_is_booked_before_the_next_symbol() -> None:
    """Бронь обязана появиться ДО перехода к следующему инструменту.

    Смысл всей затеи: снимок позиций о новой сделке не знает и до конца
    обхода не узнает. Если бронь поставить позже, следующая пара посчитает
    лимиты по устаревшему снимку."""
    print("\n[Бронь ставится внутри того же обхода]")
    исходник = (APP / "main.py").read_text(encoding="utf-8")
    без_комментариев = "\n".join(
        с.split("#", 1)[0] for с in исходник.splitlines())
    поз_ордер = без_комментариев.find("tm.execute_market_order(")
    поз_бронь = без_комментариев.find("acc_state.reservations.забронировать(")
    check(поз_ордер != -1, "Отправка ордера найдена")
    check(поз_бронь > поз_ордер,
          "Бронь идёт после отправки ордера", f"{поз_ордер} против {поз_бронь}")
    кусок = без_комментариев[поз_ордер:поз_бронь]
    # Между отправкой и бронью обязан стоять выход по неудаче. Проверяем
    # смысл, а не конкретное имя переменной: раньше здесь искалась строка
    # «if ok:», и она отвалилась от простого переименования.
    check("break" in кусок or "return" in кусок or "continue" in кусок,
          "Неудачная нога прерывает отправку, а не проваливается до брони",
          кусок.strip()[:200])
