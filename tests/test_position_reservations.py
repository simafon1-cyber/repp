...
def test_reservation_is_made_only_after_the_broker_confirms() -> None:
    ...
    # Между отправкой и бронью обязан стоять выход по неудаче. Раньше здесь
    # искалось «if ok:», но ветка успеха может быть записана и наоборот —
    # ранним выходом. Проверяем смысл, а не одну из его записей.
    check("if not ok:" in кусок or "if ok:" in кусок,
          "И только при успехе: неудача до брони не доходит", кусок.strip()[:200])
    if "if not ok:" in кусок:
        check("break" in кусок or "return" in кусок or "continue" in кусок,
              "Неудачная нога прерывает отправку, а не проваливается дальше")
