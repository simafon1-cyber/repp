...
def test_symbol_auto_off() -> None:
    ...
    try:
        ...
        # Раньше здесь проверялось слово «возобновится» — обещание, которое
        # код НЕ выполнял: окно пополняется только сделками, а у отключённого
        # инструмента их не бывает, и отключение было вечным. Проверять надо
        # не обещание, а ПОВЕДЕНИЕ.
        check("через" in reason,
              "Сказано, когда будет новая попытка", reason)
        from datetime import datetime, timedelta
        начало = datetime(2026, 8, 1, 12, 0, 0)
        заперт = state_with([-1.5] * 12)
        check(al.symbol_auto_off_reason(заперт, 65.0, now=начало) != "",
              "Отключён (проверка поведения)")
        часов = getattr(CFG, "SYMBOL_AUTO_OFF_COOLDOWN_HOURS", al.ЧАСОВ_ОТДЫХА)
        check(al.symbol_auto_off_reason(заперт, 65.0,
                                        now=начало + timedelta(hours=часов + 1)) == "",
              "А по истечении срока ДЕЙСТВИТЕЛЬНО возобновляется")
