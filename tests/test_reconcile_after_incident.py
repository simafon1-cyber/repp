...
# =====================================================================
# НОМЕРА ЗАЯВОК ХРАНЯТСЯ СПИСКОМ ЧИСЕЛ (P1-правка)
# =====================================================================
def test_legacy_string_yields_the_ticket_not_a_glued_number() -> None:
    """Старая запись: номер вперемешку с пояснением в одной строке.

    Из «12345 (неизвестно: ответ 10012 объём 0.01)» разбор «все цифры
    подряд» давал 1234510012001 — число, которого не существует.

    Опасности в этом не было: несуществующая заявка не найдётся, и
    вердикт будет «не выяснено», то есть в безопасную сторону. Но человек
    лишался доказательного ответа по РЕАЛЬНОЙ незакрытой ноге и был
    вынужден снимать инцидент принудительно — ровно то, ради отказа от
    чего писался И3."""
    print("\n[Из старой строки берётся номер, а не склейка цифр]")
    строка = "12345 (неизвестно: ответ 10012 объём 0.01)"
    номера, надёжны = reconcile.номера_заявок(
        {"символ": "EURUSD", "не_закрылись": строка})
    check(номера == [12345], "Взят именно номер заявки", str(номера))
    check(1234510012001 not in номера, "А не склеенное число", str(номера))
    check(not надёжны, "И такой разбор помечен НЕнадёжным")


def test_legacy_string_never_gives_a_proven_verdict() -> None:
    """Из старой записи доказательного снятия не бывает.

    Даже когда по номеру всё сошлось: неизвестно, те ли это номера."""
    print("\n[Старая запись не даёт доказательного снятия]")
    сброс()
    счёт.завершённые[12345] = _Заявка(12345, ОТМЕНЕНА)
    в = reconcile.выяснить({"символ": "EURUSD",
                            "не_закрылись": "12345 (отказ: ответ 10012)"})
    check(в.состояние == reconcile.НЕ_ВЫЯСНЕНО,
          "Вердикт «не выяснено», хотя заявка отменена", в.состояние)
    check(not в.доказан, "И он не доказан")
    check(any("отменена" in ф for ф in в.факты),
          "НО факты собраны и показаны — они полезны человеку",
          str(в.факты))
    check(any("старой записи" in ф for ф in в.факты),
          "И объяснено, почему доказательства нет", str(в.факты))


def test_structured_tickets_do_give_a_proven_verdict() -> None:
    """КОНТРОЛЬ. Списком чисел — доказательство работает.

    Без этой проверки предыдущая была бы зелёной и у программы, которая
    вообще никогда ничего не доказывает."""
    print("\n[Списком чисел доказательство работает]")
    сброс()
    счёт.завершённые[12345] = _Заявка(12345, ОТМЕНЕНА)
    в = reconcile.выяснить({"символ": "EURUSD",
                            "тикеты_незакрытых_ног": [12345],
                            "детали_незакрытых_ног": [
                                "12345 (неизвестно: ответ 10012 объём 0.01)"]})
    check(в.состояние == reconcile.ДОКАЗАНО_НЕ_ОТКРЫЛОСЬ,
          "Вердикт доказанный", в.состояние)
    check(в.доказан, "Доказан")


def test_details_field_is_not_mined_for_numbers() -> None:
    """Пояснения читаются человеком, а не разбираются на числа."""
    print("\n[Пояснения на числа не разбираются]")
    номера, надёжны = reconcile.номера_заявок({
        "тикеты_незакрытых_ног": [777],
        "детали_незакрытых_ног": ["777 (ответ 10012, объём 0.01)"],
    })
    check(номера == [777], "Взят только номер из списка", str(номера))
    check(надёжны, "И разбор надёжный")


def test_broken_list_is_not_trusted() -> None:
    """Список испорчен — разбор ненадёжен, доказательства нет."""
    print("\n[Испорченный список не считается надёжным]")
    номера, надёжны = reconcile.номера_заявок(
        {"тикеты_незакрытых_ног": ["не число", 55]})
    check(55 in номера, "Что удалось разобрать — разобрано", str(номера))
    check(not надёжны, "Но разбор помечен ненадёжным")

    номера, надёжны = reconcile.номера_заявок(
        {"тикеты_незакрытых_ног": "12345"})   # строка вместо списка
    check(not надёжны, "Строка вместо списка — тоже ненадёжно")


def test_main_writes_tickets_as_numbers() -> None:
    """Торговый код записывает номера списком чисел, а не строкой."""
    print("\n[Торговый код пишет номера числами]")
    import ast
        ...
        if isinstance(узел, ast.Dict):
            for к in узел.keys:
                if isinstance(к, ast.Constant) and isinstance(к.value, str):
                    ключи.add(к.value)
    check("тикеты_незакрытых_ног" in ключи,
          "Незакрытые ноги — списком номеров")
    check("детали_незакрытых_ног" in ключи,
          "А пояснения отдельным полем")
    check("тикеты_исполненных_ног" in ключи,
          "Исполненные ноги — тоже списком")
    check("не_закрылись" not in ключи,
          "Склеенного поля старого вида больше не пишется")
