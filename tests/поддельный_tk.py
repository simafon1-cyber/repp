...
# Что происходило со вкладками: ("add"|"hide", подпись).
ВКЛАДКИ = []
...
class Виджет:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._дети = []
        # Открытые вкладки этого блокнота и их подписи. Раньше tabs()
        # всегда отвечал «пусто», и проверка `str(frame) in book.tabs()`
        # в программе не проверяла ничего: любая страница считалась
        # спрятанной. Теперь подделка ведёт настоящий список.
        self._вкладки = []
        self._подписи = {}
    ...
    def add(self, ребёнок=None, **kwargs):
        """Добавить страницу в блокнот — и запомнить, что она открыта."""
        ключ = object.__repr__(ребёнок)
        подпись = str(kwargs.get("text", ""))
        if подпись:
            self._подписи[ключ] = подпись
        if ключ not in self._вкладки:
            self._вкладки.append(ключ)
        ВКЛАДКИ.append(("add", self._подписи.get(ключ, "")))
        return self

    def hide(self, ребёнок=None, *a, **k):
        """Спрятать страницу — и запомнить, что её больше не видно."""
        ключ = object.__repr__(ребёнок)
        if ключ in self._вкладки:
            self._вкладки.remove(ключ)
        ВКЛАДКИ.append(("hide", self._подписи.get(ключ, "")))
        return self

    def открытые_вкладки(self):
        """Подписи страниц, которые сейчас видно. Только для тестов."""
        return [self._подписи.get(к, "") for к in self._вкладки]

    def tabs(self):
        """Notebook.tabs() обязан вернуть перечень, а не виджет."""
        return tuple(self._вкладки)
