...
def test_manual_close_is_not_blamed_on_the_stop() -> None:
    ...
    #
    # Проверяемое требование то же самое, изменился только вызов. Раньше
    # закрытие с дашборда уходило прямым mt5c.close_position_partial, а
    # ответ брокера никак не разбирался: частичное закрытие и молчание
    # брокера считались успехом. Теперь идёт tm.close_leg, который
    # дозакрывает остаток и возвращает разобранный исход.
    ...
    поз_отправка = позиция(r"tm\.close_leg\(pos\.symbol", ИСХОДНИК_MAIN)
    check(поз_метка != -1 and поз_приказ < поз_метка < поз_отправка,
          "Отметка стоит до отправки приказа на закрытие")

    # И отдельно: прямого закрытия в обход разбора здесь больше нет.
    хвост = ИСХОДНИК_MAIN[поз_приказ:поз_приказ + 3000]
    check("mt5c.close_position_partial(pos, pos.volume)" not in хвост,
          "Закрытие с дашборда не идёт мимо разбора ответа брокера")
