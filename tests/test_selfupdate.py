...
def test_auto_apply_off_by_default() -> None:
    ...
    check(fresh.UPDATE_BRANCH == "",
          ...
          fresh.UPDATE_BRANCH)
    # РАНЬШЕ ЗДЕСЬ ПРОВЕРЯЛОСЬ ОБРАТНОЕ: автоустановка была выключена, и это
    # было правильно — программа не должна подменять себя без ведома человека.
    # Владелец изменил решение прямо: «сделай, чтобы она сама ставила
    # обновление, без моего участия, чтобы корректировал через тебя, а она
    # обновлялась». На сервере, который не перезапускают неделями, иначе не
    # доезжает вообще ничего.
    #
    # Поэтому проверка не просто перевёрнута, а перенесена на то, что теперь
    # действительно защищает: установка не должна происходить, пока открыты
    # сделки. Сделку ведут трейлинг и безубыток; остаться без них с открытой
    # позицией хуже, чем обновиться на пару часов позже.
    check(fresh.UPDATE_AUTO_APPLY is True,
          "Автоустановка включена — по прямой просьбе владельца")

    ui = (APP / "desktop_app.py").read_text(encoding="utf-8")
    periodic = ui.split("def _periodic_update_check", 1)[1].split("\n    def ", 1)[0]
    check("open_positions_count()" in periodic,
          "И при этом установка на ходу смотрит на открытые сделки")
    check("return" in periodic.split("open_now > 0", 1)[-1][:200],
          "Есть сделки — установка откладывается до следующей проверки")
