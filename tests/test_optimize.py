...
def test_unreachable_target_is_named_out_loud() -> None:
    """ПОЧЕМУ ЭТОТ ТЕСТ ПОЯВИЛСЯ. В PHASE 3 вариант «вернуть цель» дал числа
    ДО КОПЕЙКИ те же, что база. Выключатель доезжал — цель появлялась на всех
    барах, — но её ни разу не достигали: цель стоит на 4R, а половина сделок
    не доходит и до 0.6R.

    Таблица вариантов такое не покажет: она сравнивает варианты между собой и
    честно отвечает «разницы нет». Вопрос «а не бессмысленна ли сама
    конструкция» она не задаёт. Поэтому он задаётся отдельно — и обязан быть
    задан в каждом отчёте, а не только когда что-то не сошлось."""
    print("\n[Недостижимая цель называется вслух]")

    def сделка(mfe_r, риск=100.0, цель=4.0):
        return {"risk_points": риск, "mfe_points": mfe_r * риск, "tp_r": цель,
                "money": -1.0}

    # Цель на 4R, ходы — как в настоящем прогоне.
    ряд = [сделка(x) for x in (0.1, 0.3, 0.55, 0.6, 0.9, 1.2, 1.85)]
    r = optimize.reach_report(ряд)
    check(r["цель_R"] == 4.0, "Расстояние до цели измерено", str(r["цель_R"]))
    check(r["медиана_MFE_R"] == 0.6, "И типичный ход сделки тоже",
          str(r["медиана_MFE_R"]))
    check(r["дошли_бы"] == 0, "Ни одна сделка до цели не доходит")
    текст = optimize.describe_reach(r)
    check("ЦЕЛЬ НЕДОСТИЖИМА" in текст,
          "И это сказано прямо, а не спрятано в числах")

    # Достижимая цель так не помечается.
    достижимо = optimize.reach_report([сделка(x, цель=1.0)
                                       for x in (0.2, 0.9, 1.4, 2.0)])
    check(достижимо["дошли_бы"] == 2, "Достижимую цель считает верно",
          str(достижимо["дошли_бы"]))
    check("ЦЕЛЬ НЕДОСТИЖИМА" not in optimize.describe_reach(достижимо),
          "И зря не пугает")

    # Цели нет вовсе — это отдельный случай, и его тоже надо назвать.
    без_цели = optimize.reach_report([сделка(0.5, цель=0.0) for _ in range(5)])
    check("НЕТ ЦЕЛИ ВОВСЕ" in optimize.describe_reach(без_цели),
          "Отсутствие цели названо отдельно от недостижимой цели")

    check("судить не по чему" in optimize.describe_reach(
        optimize.reach_report([])), "Без сделок вывод не выдумывается")

    # Замер обязан быть в отчёте всегда, а не только при провале.
    src = (APP / "run_optimize.py").read_text(encoding="utf-8")
    check("describe_reach" in src, "Замер печатается в отчёте")

    # И движок обязан записывать то, из чего замер считается.
    ряд_свечей = свечи(700)
    итог = baseline_engine.run("EURUSD", ряд_свечей, МЕТА)
    for t in итог["trades"][:1]:
        for поле in ("risk_points", "tp_r", "sl_atr"):
            check(поле in t, f"У сделки записано поле {поле}")
...
if __name__ == "__main__":
    ...
    test_unreachable_target_is_named_out_loud()
