...
// СТОП-ЛОСС ШИРОКИЙ. По требованию владельца после серии убытков множитель
// поднят с 1.2 до 2.5 ATR (и в профилях риска тоже). Тесный стоп стоял внутри
// обычного колебания цены: сделки закрывались за 8-11 секунд, не успев никуда
// пойти. Убыток в ДЕНЬГАХ при этом НЕ вырос: объём считается от риска и
// дистанции стопа, поэтому широкий стоп даёт МЕНЬШИЙ лот (UseRiskBasedLot).
input double ATRSLMultiplier    = 2.5;   // Множитель ATR для стоп-лосса (больше = шире стоп, меньше лот)
